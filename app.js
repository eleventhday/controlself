...
function renderCTDP() {
    ...
    if (state.status === 'idle') {
        ...
        document.getElementById('btn-fail').onclick = () => {
            if(confirm("确认分心？这将重置你的主链连胜记录。")) { ctdp.fail('Distraction'); render(); }
        };
...
function handleImport(input) {
    ...
    try {
        if(confirm("确定要用新导入的国策树替换当前的吗？\n警告：当前数据将丢失。")) {
            rsip.importTree(code, 'replace');
            alert('导入成功！');
            render();
        }
    } catch(e) {
...
function renderNode(node, allNodes) {
    ...
    if (node.children && node.children.length > 0) {
        ...
        childrenContainer.className = 'mt-6 ml-4 border-l-2 border-indigo-100 pl-6 space-y-4';
    ...
    if (node.id !== 'root') {
        ...
        el.onclick = (e) => {
             ...
             rsip.updateNodeStatus(node.id, nextStatus);
             render();
        }
    }
...
function checkUrlImport() {
    ...
    if (importCode) {
        currentView = 'rsip';
        setTimeout(() => {
            if(confirm("检测到分享的国策树。是否导入？(将覆盖当前数据)")) {
                 try {
                    rsip.importTree(importCode, 'replace');
                    alert('导入成功！');
